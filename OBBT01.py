...
def solve_model(task):   
    obj_var, laborU, laborL, machineU, machineL, sense, name, TorF, Feas_p = task
    model = ConcreteModel()
...
if __name__ == '__main__':
    ...
    for number in range(1000):
        task = make_task(labor_obj, machine_obj, upper, lower, False, Fpoint)
        with Pool(processes = 4) as pool:
            results = pool.map(solve_model, task)
        solve_results(results, task, upperDummy, lowerDummy)
        ...
        update_func(results, task, upperIt, upper, lowerIt, lower)
        count = count + 1
    optimalp= opt_p()
    print(optimalp)
