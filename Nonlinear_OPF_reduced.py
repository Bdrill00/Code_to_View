...
i2xm = np.array([[1042.8], [970.2], [1009.6]])
i2xp = np.array([[-34.9], [-154.2], [85]])
i2xreal = np.zeros([i2xm.shape[0],1])
i2ximag = np.zeros([i2xm.shape[0], 1])
for i in range(i2xm.shape[0]):
    i2xreal[i,0] = i2xm[i,0]*np.cos(math.radians(i2xp[i,0]))
    i2ximag[i,0] = i2xm[i,0]*np.sin(math.radians(i2xp[i,0]))
print(i2xreal)
...
model.V1r = Var(range(n), bounds=(-9000, 9000), initialize=0)
model.V1i = Var(range(n), bounds=(-9000, 9000), initialize=0)
model.V2r = Var(range(n), bounds=(-9000, 9000), initialize=0)
...
def equality_constraint5(model, i):
    return -model.Ixr[i] + sum(Gl12[i,j]*(model.V1r[j]-model.V2r[j]) for j in range(n)) - sum(Bl12[i,j]*(model.V1i[j]-model.V2i[j]) for j in range(n))==0
def equality_constraint6(model, i):
    return -model.Ixi[i] + sum(Gl12[i,j]*(model.V1i[j]-model.V2i[j]) for j in range(n)) + sum(Bl12[i,j]*(model.V1r[j]-model.V2r[j]) for j in range(n))==0

#Relating current on primary side of the transformer to the current at the secondary side
def equality_constraint7(model, i):
    return nt*model.Ixr[i] == sum(Gtr[i,j]*((1/nt)*model.V2r[j]-model.V3r[j]) for j in range(n)) - sum(Bti[i,j]*((1/nt)*model.V2i[j]-model.V3i[j]) for j in range(n)) 
def equality_constraint8(model, i):
    return nt*model.Ixi[i] == sum(Gtr[i,j]*((1/nt)*model.V2i[j]-model.V3i[j]) for j in range(n)) + sum(Bti[i,j]*((1/nt)*model.V2r[j]-model.V3r[j]) for j in range(n))
...
#     return sum(Gtr[i,j]*((1/nt)*model.V2r[j]-model.V3r[j]) for j in range(n)) - sum(Bti[i,j]*((1/nt)*model.V2i[j]-model.V3i[j]) for j in range(n))==model.I2xr[i]
# def equality_constraint10(model, i):
#     return sum(Gtr[i,j]*((1/nt)*model.V2i[j]-model.V3i[j]) for j in range(n)) + sum(Bti[i,j]*((1/nt)*model.V2r[j]-model.V3r[j]) for j in range(n))== model.I2xi[i]

#Real and imaginary current at node 3 
def equality_constraint11(model, i):
    return -sum(Gtr[i,j]*((1/nt)*model.V2r[j]-model.V3r[j]) for j in range(n)) + sum(Bti[i,j]*((1/nt)*model.V2i[j]-model.V3i[j]) for j in range(n)) + \
...
def equality_constraint12(model, i):
    return -sum(Gtr[i,j]*((1/nt)*model.V2i[j]-model.V3i[j]) for j in range(n)) - sum(Bti[i,j]*((1/nt)*model.V2r[j]-model.V3r[j]) for j in range(n)) + \
...
def equality_constraint13(model, i):
    return -sum(Gl34[i,j]*(model.V3r[j]-model.V4r[j]) for j in range(n)) +sum(Bl34[i,j]*(model.V3i[j]-model.V4i[j]) for j in range(n)) + \
        (PL*model.V4r[i] + QL*model.V4i[i])/(model.V4r[i]**2 + model.V4i[i]**2) ==0
        
def equality_constraint14(model, i):
    return -sum(Gl34[i,j]*(model.V3i[j]-model.V4i[j]) for j in range(n)) - sum(Bl34[i,j]*(model.V3r[j]-model.V4r[j]) for j in range(n)) + \
...
names = ["V1", "V2", "V3", "V4", "Islack", "Ix"]
iterate = 0
while iterate <=len(names)-1:
